...
import matplotlib
import mplcursors
#import lib.mplcursors._mplcursors as mplcursors
import copy
...
class Grapher():
    def __init__(self, master, eleana, gui_references, callbacks):
        ...
        # Define variables for cursor_on_off
    ...
    def plot_graph(self, switch_cursors = False):
        ...
        # If indexed is True replace X values with consecutive points
        if self.eleana.gui_state.indexed_x:
            data['x'] = list(range(1, len(data['x']) + 1))
        ...
        if data['complex'] and self.eleana.selections['s_cpl'] == 'cpl':
            if self.style_second['plot_type'] == 'line':
                target_ax.plot(data['x'], data['re_y'], label=legend,
                               color=self.style_second['color_re'], linewidth=self.style_second['linewidth'],
                               linestyle=self.style_second['linestyle'])
                target_ax.plot(data['x'], data['im_y'], label=legend,
                               color=self.style_second['color_im'], linewidth=self.style_second['linewidth'],
                               linestyle=self.style_second['linestyle'])
            else:
                target_ax.scatter(data['x'], data['re_y'], label=legend,
                                  color=self.style_second['color_re'], s=self.style_second['s'],
                                  marker=self.style_second['marker'])
                target_ax.scatter(data['x'], data['im_y'], label=legend,
                                  color=self.style_second['color_im'], s=self.style_second['s'],
                                  marker=self.style_second['marker'])
        ...
        # If crosshair is activated reconnect it to the new artist
        if self.sel_cursor_mode.get() == 'Crosshair':
            # Remove old
            if self.cursor is not None:
                self.cursor.remove()

            # Create new connection to Crosshair
    ...
    **********************************'''
    ...
    def cursor_on_off(self):
        ...
        if index == 0:
            ...
            #     self.canvas.mpl_disconnect(self.free_move_binding_id)
            #     self.free_move_binding_id = None
            #
            # if self.click_binding_id is not None:
            #     self.canvas.mpl_disconnect(self.click_binding_id)
    ...
    def on_click_in_plot(self, event=None, point = None):
        ...
        if (event.inaxes is not None and (self.sel_cursor_mode.get() == 'Free select' or self.sel_cursor_mode.get() == 'Crosshair') and event.button == 1):
            ...
            number_ = str(current_nr+1) if self.style_of_annotation['number'] else ''
            self.ax.annotate(text=self.style_of_annotation['text'] + number_,
                             xy=point_selected,
